...
val testing by configuration("Used when testing") {
    sources modify { it + testSources.get() }
    resources modify { it + testResources.get() }
    scopesCompile add { ScopeTest }

    outputClassesDirectory set KeyDefaults.outputClassesDirectory("classes-test")
    outputSourcesDirectory set KeyDefaults.outputClassesDirectory("sources-test")
    outputHeadersDirectory set KeyDefaults.outputClassesDirectory("headers-test")
}
 ...
 * Enables JDWP in server mode, suspended, on port 5005, by adding relevant flags to the [runOptions].
 * Any JDWP related flags already present are removed.
 */
val debug by configuration("Enables JVM debug on any forked process") {
    runOptions modify { oldOptions ->
 ...
 * Compilation is skipped and [externalClasspath] does not include [wemi.dependency.ProjectDependency]
...
val ideImport by configuration("Configuration which omits some tasks and makes other tasks tolerate failures, to make IDE import easier") {
     ...
     */
    extend(Base) { // The change must be injected where the classpath is set originally
        internalClasspath set KeyDefaults.InternalClasspathForIdeImport
        externalClasspath set KeyDefaults.ExternalClasspathForIdeImport
    }
}
...
val jitpack by configuration("Used when building for Jitpack.io") {
    ...
    publishRepository put Repository("local-jitpack", PublishRepositoryRoot)
    ...
    publish modify { publishedResult ->
