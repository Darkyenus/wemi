...
import wemi.test.TestReport.Data
import wemi.test.TestReport.Identifier
 ...
 */
class TestReport : LinkedHashMap<Identifier, Data>(), JsonSerializable, MachineWritable {

    private fun Identifier.write(json: Json) {
    ...
    private fun JsonValue.readTestIdentifier(): Identifier {
        ...
        return Identifier(id, parentId, displayName, isTest, isContainer, tags, testSource)
    }

    override fun read(json: Json, value: JsonValue) {
        for (entry in value) {
            val identifier = entry.get("identifier").readTestIdentifier()
            val data = Data().apply { read(json, entry.get("data")) }
    ...
    /**
     * Results of run of test identified by [Identifier].
     */
    class Data : JsonSerializable {

        var status: Status = Status.NOT_RUN
        ...
        override fun write(json: Json) {
            json.writeValue("status", status, Status::class.java)
        ...
        override fun read(json: Json, value: JsonValue) {
            status = json.readValue("status", Status::class.java, value)
